...
def error():
    f1 = open('error.txt','w')
    out_seq += 'Ошибка в строке' + row_counter
    f1.write(out_seq)
    f1.close
...
def operators():
    ...
    scan()
    while name() or \
          nxtsymb in ['int', 'double', 'string', 'boolean', 'float', '{', 'function', 'if', 'for', 'while', \
...
def name():
    return nxtsymb in tokens['I'].values() or \
           nxtsymb in ['System.out.println', 'System.out.print', 'prompt']
...
# функция
def function():
    if nxtsymb != 'public static': error()
    scan()
    if not(name()): error()
    scan()
    if nxtsymb == '(':
